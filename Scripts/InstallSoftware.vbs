...
'Changes from v3.0.1 - 24/02/2017
' Modified command processing feature. Any command prefaced by 'REM ' will
'   be counted as a remark and be logged as a comment and not processed.

'Changes from v3.0.0 - 22/02/2017
' Typo and updating documentations
...
'  System logfile is: C:\Windows\AppLog
...
'  %SCRIPTPATH% = C:\Windows\CCMCache\w
...
For Each CMDListSet in CMDList
  For Each CMD in CMDListSet
    If Left(CMD,4) = "REM " then
		LogInfo "Comment : " & Mid(CMD,4)
	else 
		Call Install(CMD)
	end if
  Next
